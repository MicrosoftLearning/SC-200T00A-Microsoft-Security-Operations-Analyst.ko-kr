...
1. 연산자는 `search` 다중 테이블/다중 열 검색 환경을 제공합니다. 다음 쿼리는 연산자의 사용을 보여 줍니다`search` .
...
1. `where` 연산자는 테이블을 조건자를 충족하는 행의 하위 집합으로 필터링합니다. 다음 쿼리는 연산자의 사용을 보여 줍니다`where` .
...
1. 다음 문은 문을 사용하여 변수를 `let` 선언하는 방법을 보여 줍니다. 쿼리 창에서 다음 문을 입력하고 **실행**을 선택합니다. 
...
1. 다음 문은 문을 사용하여 `let` 동적 목록을 선언하는 방법을 보여 줍니다. 쿼리 창에서 다음 문을 입력하고 **실행**을 선택합니다. 
...
### Render 연산자를 사용하여 KQL에서 시각화 만들기
