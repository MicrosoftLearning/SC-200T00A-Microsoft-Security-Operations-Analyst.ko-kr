...
1. Microsoft Defender XDR 포털 탐색 메뉴에서 **System** 섹션을 확장하고 **Settings**을 선택한 다음 **Microsoft Defender XDR**을 선택합니다.

1. *계정* 섹션에서 **Permissions and Roles**을 선택합니다.

1. 페이지를 아래로 스크롤하여 **Go to Permissions and roles** 링크를 선택합니다.

1. *Permissions and roles* 페이지에서 **+ Create custom role**을 선택합니다.

1. *기본* 페이지에서 역할 추가 대화 상자에 다음을 입력합니다.
...
1. **Permissions** 페이지에서 다음 권한을 선택합니다.

    |사용 권한 그룹|설명|  |보안 작업|일상적인 작업을 관리하고 인시던트 및 권고에 대응|

1. *보안 작업* 팝업 페이지에서 **All read and manage permissions** 라디오 단추를 선택합니다.

1. **Apply**를 선택한 후 **Next**를 선택합니다.

1. **Assign users and data sources** 페이지에서 **Create assignment** 단추를 선택합니다.

1. *할당 추가* 대화 상자에서 다음을 입력합니다.
