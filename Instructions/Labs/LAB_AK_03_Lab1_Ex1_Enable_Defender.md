...
1. 클라우드용 Microsoft Defender 왼쪽 메뉴의 관리 아래에서 **환경 설정을** 선택합니다.

1. **“Azure Pass - 스폰서쉽”** 구독(또는 사용 중인 언어로 동일한 이름)을 선택합니다. 

1. 이제 Defender for Cloud 플랜으로 보호되는 Azure 리소스를 검토합니다.
...
1. **모든 클라우드용 Microsoft Defender 플랜 사용**을 선택하고 **저장**을 선택합니다. *"작업 영역 uniquenameDefender에 대한 Microsoft Defender 계획이 성공적으로 저장되었습니다!"* 기다립니다.
...
1. 입력: cd C:\Users\Administrator\Downloads
    ...
    >**중요:** 이 디렉터리가 없는 경우 사용자가 잘못된 컴퓨터에 있을 가능성이 높습니다. 작업 4의 시작 부분으로 돌아가기 WINServer로 변경하고 다시 시작합니다.
...
1. 설치 프로세스에서 새 Edge 브라우저 탭을 열어 Azure Arc 에이전트를 인증합니다. 관리자 계정을 선택하고 "인증 완료" 메시지가 표시되기를 기다린 후 Windows PowerShell 창으로 돌아갑니다.

1. 스크립트를 다운로드한 Azure Portal 페이지로 돌아가서 **닫기**를 선택합니다. **Azure Arc를 사용하여 서버 추가**를 닫고 Azure Arc **서버** 페이지로 돌아갑니다.

1. WINServer 서버 이름이 표시되고 상태가 *연결*될 때까지 **새로 고침**을 선택합니다.
