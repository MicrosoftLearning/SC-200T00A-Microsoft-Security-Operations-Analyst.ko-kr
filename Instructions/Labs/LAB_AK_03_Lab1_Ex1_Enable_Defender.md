...
### 작업 1: Azure Portal에 액세스하고 구독 설정
...
1. Azure Portal의 검색 창에서 구독*을 입력*한 다음 구독을** 선택합니다**. 

1. *표시된 "Azure Pass - 스폰서쉽"* 구독(또는 선택한 언어의 해당 이름)을 선택합니다.

    >**참고:** 구독이 표시되지 않으면 강사에게 테넌트 관리자 사용자 자격 증명을 사용하여 Azure 구독을 만드는 방법에 대해 문의하세요. **참고:** 구독 만들기 프로세스는 최대 10분이 걸릴 수 있습니다. 

1. **액세스 제어(IAM)** 를 선택한 다음, 이 리소스* 상자에 대한 액세스 권한 부여 상자에서 *역할 할당** 추가를 선택합니다**.

1. 권한 있는 **관리자 역할** 탭을 선택한 다음 소유자를** 선택합니다**. **다음**을 선택하여 작업을 계속할 수 있습니다.

1. 구성원 탭에서 *+ 멤버 **** 를 선택하고 **MOD 관리istrator** 계정을 선택하고 선택을** 선택하여 **계속합니다.*

    >**참고:** 조건 탭에 **빨간색 점이 표시되면 다음**을 선택하고 **위임* 유형으로 제공된** 경우 제한되지 않음을 선택**하거나 사용자가 수행할* 수 있는 작업과 함께 ** 제공된 경우 모든 역할(높은 권한)**을 할당하도록 허용을 선택합니다**.**

1. 검토 + 두 번 할당**을 선택하여 **관리자 계정에 소유자 역할을 할당합니다.

>**중요:** 이러한 랩은 수업 중에 10달러 미만의 Azure 서비스를 사용하도록 설계되었습니다.
...
1. Azure Portal의 검색 창에서 Log Analytics 작업 영역을* 입력*한 다음, 동일한 서비스 이름을 선택합니다.

1. 명령 모음에서 +만들기**를 선택합니다**.

1. 리소스 그룹에 대해 새로** 만들기를 선택합니다**.
...
1. 작업 영역 유효성 검사가 통과되면 만들기**를 선택합니다**. 새 작업 영역이 프로비전될 때까지 기다립니다. 이 작업은 몇 분 정도 걸릴 수 있습니다.
...
1. **시작** 페이지의 **업그레이드** 탭 아래에서 구독이 선택되어 있는지 확인한 다음, 페이지 아래쪽에서 **업그레이드** 단추를 선택합니다. 평가판 시작 알림이 ** 표시될 때까지 기다렸다가 약 2분이 걸립니다. 

    >**힌트:** 위쪽 표시줄에서 벨 단추를 클릭하여 Azure Portal 알림을 검토할 수 있습니다.
...
1. 모니터링 확장을 검토합니다. 여기에는 Virtual Machines, 컨테이너 및 스토리지 계정에 대한 구성이 포함됩니다. 페이지의 오른쪽 위에 있는 'X'를 선택하여 "설정 및 모니터링" 페이지를 닫습니다.
...
1. **모든 계획** 사용(Defender 플랜 선택 오른쪽)을 선택한 다음, 저장**을 선택합니다**. *"작업 영역 uniquenameDefender에 대한 Microsoft Defender 계획이 성공적으로 저장되었습니다!"를 기다립니다.* 기다립니다.

    >**참고:** 페이지가 표시되지 않는 경우 Edge 브라우저를 새로 고치고 다시 시도합니다.
...
이 작업에서는 WINServer** 가상 머신에 DCR(데이터 수집 규칙)*을 추가하여 *Azure Monitor 에이전트*를 **수동으로 설치*합니다.

1. **클라우드용 Microsoft Defender** 이동하여 왼쪽 메뉴에서 시작** 페이지를 선택합니다**.

1. **시작** 탭을 선택합니다.

1. 아래로 스크롤하여 비 Azure 서버* 추가 섹션에서 구성 *** 을 선택합니다**.

1. 이전에 만든 작업 영역 옆에 있는 업그레이드**를 선택합니다**. 몇 분 정도 걸릴 수 있습니다. "작업 영역 uniquenameDefender에 대한 Microsoft Defender 계획이 성공적으로 저장되었습니다!"*라는 알림*이 표시될 때까지 기다립니다.

1. 이전에 만든 작업 영역 옆에 있는 + 서버** 추가를 선택합니다**.

1. 데이터 수집 규칙 선택 ****

1. **+ 만들기**를 선택합니다.

1. 규칙 이름에 WINServer**를 입력**합니다.

1. *Azure Pass - 스폰서쉽* 구독을 선택하고 리소스 그룹을 선택합니다. **힌트:** *RG-Defender*

1. 기본 *미국* 동부 지역을 유지하거나 다른 바람직한 위치를 선택할 수 있습니다.

1. 플랫폼 유형에 대한 Windows 라디오 단추를 선택하고 다음:리소스를** 선택합니다**.** **** 

1. 리소스 탭에서 **+ 리소스** 추가 ****

1. 범위 선택 페이지에서 RG-Defender **(또는 만든 리소스 그룹)에 대한 **범위* 열을 확장*한 다음, WINServer**를 선택하고 **적용**을 선택합니다**.** ** 

    >**참고:** WINServer**가 표시되지 않는 경우 **리소스 종류*에 대한 *열 필터를 *Server-Azure Arc*로 설정해야 할 수 있습니다.

1. 다음:수집 및 배달 선택 ****

1. **수집 및 배달** 탭에서 + 데이터 원본 추가를 선택합니다 **.**

1. **데이터 원본 추가 페이지에서 데이터 원본** 형식*에서 *성능 카운터를** 선택합니다**.

    >**참고:** 이 랩에서는 Windows 이벤트 로그를 선택할 *수 있습니다*. 이러한 선택은 나중에 수정할 수 있습니다.

1. 데이터 원본** 추가를 선택하고 검토 + 만들기를 선택합니다.******

1. 유효성 검사가 통과된 후 *만들기**를* 선택합니다**.

1. 데이터 수집 규칙 만들기는 **WINServer**에 *AzureMonitorWindowsAgent* 확장 설치를 **시작합니다.**

1. 데이터 수집 규칙 만들기가 *완료되면 검색 리소스, 서비스 및 문서 검색 창에 *WINServer**를* 입력**하고 리소스에서 *WINServer를*** 선택합니다**.*

1. WINServer에서 **왼쪽 메뉴를 아래로 스크롤하여 *설정* 확장*입니다*.**

1. **AzureMonitorWindowsAgent**는 성공** 상태 *** 와 함께 *나열되어야 합니다.

1. 다음 랩으로 이동하고 나중에 돌아가서 클라우드용 Microsoft Defender** 인벤토리** 섹션**을 검토**하여 WINServer**가 포함되어 있는지 **확인할 수 있습니다.
