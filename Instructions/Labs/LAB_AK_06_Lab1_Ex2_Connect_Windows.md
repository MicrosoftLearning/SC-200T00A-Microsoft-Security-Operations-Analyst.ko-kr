...
# <a name="learning-path-6---lab-1---exercise-2---connect-windows-devices-to-microsoft-sentinel-using-data-connectors"></a>학습 경로 6 - 랩 1 - 연습 2 - 데이터 커넥터를 사용하여 Microsoft Sentinel에 Windows 디바이스 연결
...
1. 아래로 스크롤하여 가상 머신의 크기를 검토합니다. 비어 있는 것 같은 경우 **모든 크기 보기**를 선택하고 Azure 사용자들이 가장 많이 사용함에서 첫 번째 VM 크기를 선택하고 **선택**을 클릭합니다.
...
1. Windows 시작 단추를 마우스 오른쪽 단추로 클릭하고 **Windows PowerShell(관리자)** 을 선택합니다.
    ...
    >**참고:** **Windows PowerShell**을 검색해야 할 수도 있습니다. 검색 상자에 **PowerShell**을 입력합니다. **Windows PowerShell 앱**이 표시됩니다. **관리자 권한으로 실행** 옵션을 선택합니다.

1. UAC 프롬프트가 표시되면 “Username” 및 *Passw0rd!* 에 대해 *Administrator*를 입력합니다. “암호” 경우 다음 단계로 건너뜁니다.
...
1. *.\OnboardingScript.ps1*을 입력하고 Enter 키를 누릅니다.  
    ...
    >**중요:** *".\OnboardingScript.ps1 용어가 인식되지 않음..."* 오류가 표시되는 경우 WIN2 가상 머신에서 작업 3에 대한 단계를 수행하고 있는지 확인합니다. 여러 다운로드로 인해 파일 이름이 변경되는 다른 문제가 있을 수 있습니다. 실행 중인 디렉터리에서 *“.\OnboardingScript (1).ps1”* 또는 다른 파일 번호를 검색합니다.
