...
1. 다운로드한 파일의 이름 아래에서 **파일 열기** 를 선택한 다음 **, 추가 앱을** 선택합니다.

1. **메모장을** 선택한 다음 **확인을** 선택합니다.
...
1. *Azure DevOps 시작* 페이지에서 **새 organization 만들기**를 선택한 다음 **, 계속**을 선택합니다.
...
1. 파일(페이지의 오른쪽) 블레이드에서 도구 모음에는 *빌드 설정*, *복제*, ... 옵션이 포함됩니다. 콜론 아이콘 **(:)** 을 선택하여 추가 옵션을 표시합니다.
...
1. *애플리케이션 연결 정책* 영역에서 *OAuth를 통해 타*사 애플리케이션 액세스를 **토글합니다**.
