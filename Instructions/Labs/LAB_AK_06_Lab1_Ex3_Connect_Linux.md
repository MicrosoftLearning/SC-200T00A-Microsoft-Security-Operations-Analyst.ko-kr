...
1. *콘텐츠 허브*에서 **공통 이벤트 형식** 솔루션을 검색하고 목록에서 선택합니다.

1. *일반 이벤트 형식* 솔루션 페이지에서 **설치**를 선택합니다.

1. 설치가 완료되면 **관리를** 선택합니다.

    >**참고:** *Common Event Format* 솔루션은 *AMA를 통해 CEF(Common Events Format)* 와 *CEF(Common Events Format)* 데이터 커넥터를 모두 설치합니다.

1. *CEF(Common Events Format)* 데이터 커넥터를 선택하고 커넥터 정보 블레이드에서 **커넥터 열기 페이지를** 선택합니다.

1. *구성* 섹션의 *지침* 탭에서 *1.2 Linux 머신에 CEF 수집기 설치에* 표시된 명령을 클립보드에 복사합니다.
...
1. *Syslog* 데이터 커넥터를 선택하고 커넥터 정보 블레이드에서 **커넥터 페이지 열기**를 선택합니다.

1. *구성* 섹션에서 **비 Azure Linux 컴퓨터에 에이전트 설치를 확장합니다**.
