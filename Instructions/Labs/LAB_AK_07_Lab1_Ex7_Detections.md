...
# <a name="learning-path-7---lab-1---exercise-7---create-detections"></a>학습 경로 7 - 랩 1 - 연습 7 - 검색 만들기
...
이 작업에서는 엔드포인트용 Microsoft Defender가 구성되어 있는 호스트(Win1)에서 **공격 1** 검색을 만듭니다.
...
1. 이미 일반화되어 쉽게 쿼리할 수 있는 데이터는 *DeviceRegistryEvents* 테이블에 포함되어 있는 것으로 보입니다. 행을 확장하여 레코드와 관련된 모든 열을 표시합니다.
    ...
    >**중요:** 결과에 *DeviceRegistryEvents* 테이블이 표시되지 않는 경우 다음 두 쿼리의 대안은 *DeviceProcessEvents* 테이블을 대체로 사용하는 것입니다. 즉, 이전 쿼리에 표시된 테이블에 따라 아래 제공된 두 예제 중 하나를 사용합니다.
...
이 작업에서는 보안 이벤트 커넥터가 설치되어 있는 호스트(Win2)에서 **공격 2** 검색을 만듭니다.
