...
1. 결과는 세 가지 테이블의 데이터를 보여 줍니다.
    ...
    >**참고:** 드문 경우지만 초기 데이터 로드 프로세스는 예상보다 오래 걸립니다. 이 경우 몇 시간 동안 결과에 테이블이 표시되지 않습니다. 이벤트 테이블만 표시되는 경우 지금 사용할 이벤트 테이블이므로 계속 진행할 수 있습니다.
...
1. 인시던트 설정(미리 보기) 탭에서 기본값을 그대로 두고 **다음: 자동화된 응답 >** 단추를 선택합니다.
